...
export async function getAlbumInfo(
  ...
  try {
    ...
    if (!contentType || !contentType.includes("application/json")) {
      ...
      throw new Error(
        "Last.fm API returned non-JSON response. Check your API key and rate limits.",
      );
    }
...
export async function enrichAlbumsWithImages(
  albums: LastFmAlbum[],
  apiKey: string,
): Promise<LastFmAlbum[]> {
    ...
    albums.map(async (album) => {
      try {
        ...
        if (albumInfo?.image?.length > 0) {
          ...
          const hasValidImage = albumInfo.image.some(
            (img) => img["#text"] && img["#text"].length > 0,
          );
          if (hasValidImage) {
            return {
              ...album,
...
export async function enrichArtistsWithInfo(
  artistList: LastFmArtist[],
  apiKey: string,
): Promise<LastFmArtist[]> {
...
export async function getTrackInfo(
  ...
  try {
    ...
    console.error(
      `Error fetching track info for ${artistName} - ${trackName}:`,
      error,
    );
    return { tags: [], info: null };
  }
...
export async function getRecentTracksWithTags(
  apiKey: string,
  limit: number = 5,
): Promise<{
  ...
  try {
    ...
    if (
      ...
      if (recentTrackTags.length === 0) {
        recentTrackTags = await getArtistTopTags(
          recentTracks.artist["#text"],
          apiKey,
        );
      }
