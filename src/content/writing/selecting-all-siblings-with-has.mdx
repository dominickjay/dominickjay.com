...
<div class="fyi-block fyi-block--success">
  <span>Update - 23rd December, 2024</span>
...
<style>{`
  #has-example-row-4 > * {
    background-color: color-mix(in srgb, var(--feature-color) 100%, transparent);;
  }
`}</style>
...
<style>{`
  ...
  #has-example-row-6:has( *:hover ) > *:not(:hover) {
    background-color: var(--feature-color, #ff5f1f);
  }
`}</style>
